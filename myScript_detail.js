  ...
  $.get("db.json", function (produkt) {
    ...
    for (let i = 0; i < produkt.length; i++) {
      ...
      if (produktId == idUrl) {
        ...
        $('#detail').append(`<img class="zoom4" src="${produkt[i].img}"></img><h2>${produkt[i].nazov}</h2><h3>${produkt[i].nazov1}</h3><h4>Price: ${produkt[i].cena} eur</h4><button idproduktu=1 class="btn btn-default zoom1" type="submit">Add to <span class="glyphicon glyphicon-shopping-cart"></span></button><b class="nasklade">Aviable: ${produkt[i].nasklade}ks</b><p><b>Description:</b><br><br>${produkt[i].popis}</p>`);
      }
    }
